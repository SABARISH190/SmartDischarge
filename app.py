...
def index():
    try:
        ...
        return f"An error occurred while loading the index page: {str(e)}", 500
...
def add_patient():
    try:
        ...
        logger.error(f"Error in add_patient route: {str(e)}")
...
def add_patient_from_data(data, session, next_id, test_report_path=''):
    """Helper function to create a patient from form or JSON data"""
    # Convert string boolean to actual boolean
    has_chronic_condition = data.get('has_chronic_condition')
    if isinstance(has_chronic_condition, str):
        has_chronic_condition = has_chronic_condition.lower() == 'true'
    
    # Get admission and discharge dates, default to today for discharge if not provided
    admission_date = data.get('admission_date')
    discharge_date = data.get('discharge_date')
    
    # If admission date is not provided, set it to today
    if not admission_date and discharge_date:
    ...
    # If discharge date is not provided, set it to admission date + 1 day
    if not discharge_date:
        discharge_date = (datetime.strptime(admission_date, '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d')
    
    # Safely handle hospital_stay_duration
    stay_duration = data.get('hospital_stay_duration', '1')
    try:
        stay_duration_int = int(stay_duration) if stay_duration.strip() else 1
    except ValueError:
        logger.error("Invalid hospital_stay_duration: %s", stay_duration)
        raise ValueError("Hospital stay duration must be a valid number")
    ...
    new_patient = Patient(
        ...
        GeneralHealth=data.get('general_health', 'Unknown'),
        HasChronicCondition=has_chronic_condition,
        HospitalStayDuration=stay_duration_int,
        RiskCategory=data.get('risk_category', 'Unknown'),
        DoctorName=data.get('doctor_name', 'Dr. Anita Sharma'),
        Allergies=data.get('allergies', 'None'),
        AdmissionDate=admission_date,
        DischargeDate=discharge_date,
        ChiefComplaint=data.get('chief_complaint', 'Unknown'),
...
def add_patient_post():
    ...
    try:
        ...
        # Check if patient ID is provided in the form
        provided_id = request.form.get('patient_id')
        if provided_id and provided_id.isdigit():
            existing_patient = session.query(Patient).filter(Patient.PatientID == int(provided_id)).first()
            if existing_patient:
                return jsonify({'error': f'Patient with ID {provided_id} already exists'}), 400
            next_id = int(provided_id)
        ...
        new_patient = add_patient_from_data(request.form, session, next_id, test_report_path)
        ...
        session.rollback()
        logger.error("IntegrityError adding patient: %s", str(e))
        return jsonify({
            'success': False,
            'error': 'Database error: Patient ID or data conflict'
        }), 400
    except ValueError as ve:
        session.rollback()
        logger.error("ValueError adding patient: %s", str(ve))
        return jsonify({
            'success': False,
            'error': str(ve)
        }), 400
        ...
        session.rollback()
        logger.error("Unexpected error adding patient: %s", str(e))
        return jsonify({
            'success': False,
...
if __name__ == '__main__':
    try:
        ...
        app.run(debug=True, host='0.0.0.0', port=port)  # Enable debug mode
    except Exception as e:
